...
describe("DNS — Mode 2 (domain only, no Route 53)", () => {
    ...
    it("outputs DKIM CNAME records as CfnOutput", () => {
        ...
        const dkimOutputs = Object.keys(outputs).filter((k) =>
            k.includes("DkimCname"),
        );
    ...
    it("outputs SPF record as CfnOutput", () => {
        const outputs = template.toJSON().Outputs ?? {};
        const spfOutputs = Object.keys(outputs).filter((k) =>
            k.includes("SpfRecord"),
        );
    ...
    it("outputs DMARC record as CfnOutput", () => {
        const outputs = template.toJSON().Outputs ?? {};
        const dmarcOutputs = Object.keys(outputs).filter((k) =>
            k.includes("DmarcRecord"),
        );
    ...
    it("outputs MX record as CfnOutput", () => {
        const outputs = template.toJSON().Outputs ?? {};
        const mxOutputs = Object.keys(outputs).filter((k) =>
            k.includes("MxRecord"),
        );
