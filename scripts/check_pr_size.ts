...
const filteredList = diffFileList.filter((file) => !EXCLUDE.find((e) => file.includes(e)));
...
const shortStatArgs = ["diff", "--shortstat", baseRef, "HEAD", "--", ...filteredList];
...
if (shortStatOutputString.includes("insertion")) {
    ...
    if (!insertionsCount) {
        throw new Error(`Unable to parse insertion count from ${shortStatOutputString}`);
    }
    linesAdded = parseInt(insertionsCount, 10);
}
if (shortStatOutputString.includes("deletion")) {
    const deletionsCount = shortStatOutputString.match(/(\d+)\s+deletion/)?.[1];
    if (!deletionsCount) {
        throw new Error(`Unable to parse deletions count from ${shortStatOutputString}`);
    }
    linesRemoved = parseInt(deletionsCount, 10);
}
