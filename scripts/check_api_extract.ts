...
const updatedAPIFiles = gitDiffFiles
    .toString()
    .split("\n")
    .filter((file) => file.includes("API.api.md"));

if (updatedAPIFiles.length > 0) {
    throw new Error(
        `Expected no API.api.md file updates but found ${updatedAPIFiles.join(
            ", ",
        )}\nRun 'npm run update:api' and commit the updates to fix.`,
    );
}

const allApiExtractFiles = await glob("packages/*/API.api.md");
const errors: string[] = [];
...
if (errors.length > 0) {
    const errorMessagePrefix = `Problems found in API.api.md files.\nFix these issues and regenerate the API.api.md files using 'npm run update:api'`;
