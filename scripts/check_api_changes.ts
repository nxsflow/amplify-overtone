...
/**
 * This script checks whether current (aka latest) repository content
...
const latestRepositoryPath = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "..");
...
// Types that are too complex to validate or cause false positives
const excludedTypesByPackageName: Record<string, Array<string>> = {
    // Example: 'amplify-overtone': ['SomeComplexType'],
};

const validationResults = await Promise.allSettled(
    packagePaths.map(async (packagePath) => {
        const packageName = path.basename(packagePath);
        const baselinePackagePath = path.join(baselineRepositoryPath, "packages", packageName);
        const baselinePackageApiReportPath = path.join(baselinePackagePath, "API.api.md");
        if (!existsSync(baselinePackageApiReportPath)) {
            console.log(`Skipping ${packageName} as it does not have baseline API.api.md file`);
            return;
        }
