...
export class UsageStatementsRenderer {
    /**
     * Creates usage statements renderer.
     */
    constructor(
     ...
     *
     * This is a cheap alternative to a proper but more expensive solution
    ...
    private applyNamespaces = (usageStatements: string): string => {
        for (const symbolName of this.namespaceDefinitions.namespaceBySymbol.keys()) {
            ...
            const symbolAlias = this.namespaceDefinitions.aliasedSymbols.get(symbolName);
            let actualSymbolName = symbolName;
            if (symbolAlias) {
                actualSymbolName = symbolAlias;
            }
            const symbolWithNamespace = `${namespaceHierarchy.join(".")}.${actualSymbolName}`;
            ...
            usageStatements = usageStatements.replaceAll(regex, `$1${symbolWithNamespace}$3`);
        }
