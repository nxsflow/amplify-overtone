...
import type { NamespaceDefinitions, UsageStatementsGeneratorOutput } from "./types.js";
import { UsageStatementsRenderer } from "./usage_statements_renderer.js";

/**
 * Generates API usage using API.api.md definition.
 */
    ...
    private generateStatementsForNode = (
        ...
        switch (node.kind) {
            case ts.SyntaxKind.ImportDeclaration:
                return new ImportUsageStatementsGenerator(node as ts.ImportDeclaration).generate();
                ...
                if (moduleDeclaration.body?.kind === ts.SyntaxKind.ModuleBlock) {
                    ...
                    for (const moduleBodyStatement of moduleBody.statements) {
                        if (moduleBodyStatement.kind === ts.SyntaxKind.ExportDeclaration) {
                            const exportDeclaration = moduleBodyStatement as ts.ExportDeclaration;
                            if (
                                exportDeclaration.exportClause?.kind === ts.SyntaxKind.NamedExports
                            ) {
                                const namedExports =
