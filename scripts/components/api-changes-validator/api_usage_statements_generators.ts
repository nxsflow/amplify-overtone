...
import type { UsageStatementsGenerator, UsageStatementsGeneratorOutput } from "./types.js";
    ...
    implements UsageStatementsGenerator
{
     ...
     */
    constructor(
        private readonly typeParameters: ts.NodeArray<ts.TypeParameterDeclaration> | undefined,
    ) {}
    ...
    generate = (): UsageStatementsGeneratorOutput => {
        ...
        return {
            usageStatement: `<${this.typeParameters.map((item) => item.getText()).join(", ")}>`,
        };
    };
}
...
export class GenericTypeParameterUsageStatementsGenerator implements UsageStatementsGenerator {
    ...
    generate = (): UsageStatementsGeneratorOutput => {
        ...
        return {
            usageStatement: `<${this.typeParameters.map((item) => item.name.getText()).join(", ")}>`,
        };
    };
}
 ...
 *
 * This is a pass through process. Imports present in API.api.md report are
...
/**
 * Generates statements for types defined in API.api.md.
 *
...
export class TypeUsageStatementsGenerator implements UsageStatementsGenerator {
    ...
    generate = (): UsageStatementsGeneratorOutput => {
        ...
        usageStatement += `const ${toLowerCamelCase(typeName)}UsageFunction = ${genericTypeParametersDeclaration}(${functionParameterName}: ${baselineTypeName}${genericTypeParameters}) => {${EOL}`;
 ...
 * like we do for types we generate usage of its members.
 */
...
class ClassConstructorUsageStatementsGenerator implements UsageStatementsGenerator {
     ...
     */
    private generateConcreteClassConstructorUsage = (): UsageStatementsGeneratorOutput => {
        const className = this.classDeclaration.name?.getText();
        if (!className) {
            throw new Error("Class name is missing");
        }
        const usageFunctionName = toLowerCamelCase(`${className}ConstructorUsageFunction`);
     ...
     * and call its constructor via super() call with matching list of parameters.
     */
    private generateAbstractClassConstructorUsage = (): UsageStatementsGeneratorOutput => {
        const className = this.classDeclaration.name?.getText();
        if (!className) {
            throw new Error("Class name is missing");
        }
...
class ClassInheritanceUsageStatementsGenerator implements UsageStatementsGenerator {
    ...
    generate = (): UsageStatementsGeneratorOutput => {
        ...
        const usageFunctionName = toLowerCamelCase(`${className}InheritanceUsageFunction`);
...
export class VariableUsageStatementsGenerator implements UsageStatementsGenerator {
    ...
    generate = (): UsageStatementsGeneratorOutput => {
        ...
        const variableDeclaration = this.variableStatement.declarationList
    ...
    implements UsageStatementsGenerator
{
     ...
     * @inheritDoc
     */
    constructor(private readonly parameters: ts.NodeArray<ts.ParameterDeclaration>) {}

    generate = (): UsageStatementsGeneratorOutput => {
        const usageStatement = this.parameters.map((parameter) => parameter.getText()).join(", ");
        return {
            usageStatement,
        };
    };
