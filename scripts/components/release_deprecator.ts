...
import type { DistTagMover } from "./dist_tag_mover.js";
import type { GitClient } from "./git_client.js";
import type { GithubClient } from "./github_client.js";
import type { NpmClient } from "./npm_client.js";

/**
 * Orchestrates the process of marking the packages in a release as deprecated
 * and moving related npm dist-tags back to the previous package versions.
 */
export class ReleaseDeprecator {
     ...
     * 3. Find the git tags associated with the previous versions of the packages that are being deprecated.
     *    These are the package versions that need to be marked as "latest" (or whatever the dist-tag for the release is)
     * 4. Creates a rollback PR that resets the .changeset directory to its state before the release
     * 5. Resets the dist-tags to the previous package versions
     * 6. Marks the current package versions as deprecated
     */
    deprecateRelease = async () => {
        await this.gitClient.ensureWorkingTreeIsClean();

        const releaseCommitHashToDeprecate = await this.gitClient.getNearestReleaseCommit(
        ...
        await this.gitClient.checkout(`${releaseCommitHashToDeprecate}^`, [".changeset"]);
