...
import { type PackageJson, readPackageJson } from "./package_json.js";
...
type DependencyVersionPredicate = (declarations: DependencyDeclaration[]) => true | string;
...
export class DependenciesValidator {
    ...
    private async validateDependencyVersionsConsistency(): Promise<void> {
            ...
            [
                ...
                if (dependency) {
                    for (const dependencyName of Object.keys(dependency)) {
                        ...
                        let dependencyVersionsUsage = dependencyVersionsUsages[dependencyName];
                        if (!dependencyVersionsUsage) {
                            dependencyVersionsUsage = {
                                allDeclarations: [],
                            };
                            dependencyVersionsUsages[dependencyName] = dependencyVersionsUsage;
                        }
        ...
        for (const linkedDependencySpec of this.linkedDependencies) {
            ...
            for (const dependencyName of linkedDependencySpec) {
                ...
                if (dependencyVersionUsage) {
                    for (const { version } of dependencyVersionUsage.allDeclarations) {
    ...
    private async validateDependencyDenyAndAllowListRules(): Promise<void> {
        const violations: Array<DependencyViolation> = (
            await Promise.all(
                this.packagePaths.map((packagePath) => this.checkPackageDependencies(packagePath)),
            )
        ).flat();
    ...
    /**
     * Checks dependencies of a package located at packagePath against provided rules.
     */
    private async checkPackageDependencies(
        ...
        const allDependencies = this.collectAllDependenciesRecursively(npmListResult);
        ...
        for (const dependencyName of allDependencies) {
            ...
            if (dependencyName in this.disallowedDependencies) {
                const dependencyRule = this.disallowedDependencies[
                    dependencyName
                ] as DependencyRule;
                const isViolating =
                    dependencyRule.denyAll || !dependencyRule.allowList.includes(packageName);
                if (isViolating) {
                    violations.push({
                        packageName,
                        dependencyName,
     ...
     */
    private collectAllDependenciesRecursively(npmListOutput: NpmListOutputItem): Set<string> {
        const dependencies: Set<string> = new Set();

        if (npmListOutput.dependencies) {
            for (const dependencyName in npmListOutput.dependencies) {
                dependencies.add(dependencyName);
                const depItem = npmListOutput.dependencies[dependencyName];
                const nestedDependencies = depItem
                    ? this.collectAllDependenciesRecursively(depItem)
                    : new Set<string>();
                for (const item of nestedDependencies) {
    ...
    private getPackageVersionDeclarationPredicate = async (
        ...
        const inconsistentDependency = this.knownInconsistentDependencyVersions.find(
        ...
        if (inconsistentDependency) {
            return (declarations) => {
                ...
                return (
                    ...
                    `${packageName} dependency declarations must depend on version ${
                        ...
                        inconsistentDependency.exceptions
                            .map(
                                (exception) =>
            ...
            return (declarations) => {
                ...
                const baselineMajorVersion = declarations.at(0)!.version.split(".").at(0)!;
        ...
        return (declarations) => {
            ...
            return (
                versionSet.size === 1 ||
                `${packageName} dependency declarations must all be on the same semver range`
            );
        };
    };
