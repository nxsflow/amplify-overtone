...
export class ReleaseRestorer {
    /**
     * Initialize with release restore config and necessary clients.
     */
    constructor(
     ...
     *
     * Running this method without running the deprecateRelease method is effectively a no-op
     * (because the current release is already "un-deprecated").
     */
    restoreRelease = async () => {
        await this.gitClient.ensureWorkingTreeIsClean();

        const releaseCommitHashToRestore = await this.gitClient.getNearestReleaseCommit(
        ...
        await this.distTagMover.moveDistTags(previousReleaseTags, releaseTagsToRestore);
    };
}
