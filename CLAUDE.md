...
| `pnpm test-infra:deploy`    | Deploy test infra (writes `amplify_outputs.json` in-place) |
...
- **Test infra outputs**: `packages/test-infra/amplify_outputs.json` (generated by `pnpm test-infra:deploy`, gitignored). Test user credentials are in Secrets Manager, fetched at runtime by `loadTestInfraConfig()`.
